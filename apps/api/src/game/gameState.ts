...
function parseRecordValue<T>(recordValue: string | T): T {
    return typeof recordValue === 'string' ? JSON.parse(recordValue) : recordValue;
}

function parseLastAttack(recordValue: PlayerRecord['last_attack']): AttackResult {
    const lastAttack = parseRecordValue<AttackResult | null>(recordValue);
    return lastAttack ?? createEmptyAttackResult();
}
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players = [{
                ...
                board_data: parseRecordValue<Board>(playerRecords[0].board_data),
                attack_data: parseRecordValue<Board>(playerRecords[0].attack_data),
                ship_data: parseRecordValue<ShipData>(playerRecords[0].ship_data),
                ...
                board_data: parseRecordValue<Board>(playerRecords[1].board_data),
                attack_data: parseRecordValue<Board>(playerRecords[1].attack_data),
                ship_data: parseRecordValue<ShipData>(playerRecords[1].ship_data),
