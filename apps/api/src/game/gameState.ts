...
const emptyLastAttack: AttackResult = {
    position: null,
    result: null,
    target: null
};

function parseRecordField<T>(value: string | T): T {
    return typeof value === 'string' ? JSON.parse(value) : value;
}

function parseLastAttack(value: string | AttackResult | null): AttackResult {
    if (value === null) {
        return { ...emptyLastAttack };
    }
    const lastAttack = parseRecordField<AttackResult>(value);
    return lastAttack ?? { ...emptyLastAttack };
}
...
class GameStateController {
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players = [
                convertPlayerRecordToPlayer(playerRecords[0]),
                convertPlayerRecordToPlayer(playerRecords[1])
            ];
            return players;
        });
        const gameState = {
