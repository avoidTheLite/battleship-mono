...
const defaultLastAttack: AttackResult = {
    position: null,
    result: null,
    target: null
};
...
function normalizeLastAttack(value: AttackResult | string | null): AttackResult {
    if (!value) {
        return { ...defaultLastAttack };
    }

    const parsedValue = parseJsonField<AttackResult | null>(value);
    return parsedValue ?? { ...defaultLastAttack };
}
...
class GameStateController {
    ...
    public async createGame(players: PlayerBase[], gameName: string): Promise<GameState> {
        ...
        const player1: PlayerRecord = {
            ...
            last_attack: JSON.stringify(defaultLastAttack)
        }
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players = playerRecords.map((playerRecord) => ({
                id: playerRecord.id,
                ...
                last_attack: normalizeLastAttack(playerRecord.last_attack)
            }));
