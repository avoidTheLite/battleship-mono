...
function parsePersistedJson<T>(value: T | string | null | undefined, fallback?: T): T {
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players = [{
                ...
                board_data: parsePersistedJson<Board>(playerRecords[0].board_data),
                attack_data: parsePersistedJson<Board>(playerRecords[0].attack_data),
                ship_data: parsePersistedJson<ShipData>(playerRecords[0].ship_data),
                last_attack: parsePersistedJson<AttackResult>(playerRecords[0].last_attack, createEmptyAttackResult())
            },{
                ...
                board_data: parsePersistedJson<Board>(playerRecords[1].board_data),
                attack_data: parsePersistedJson<Board>(playerRecords[1].attack_data),
                ship_data: parsePersistedJson<ShipData>(playerRecords[1].ship_data),
                last_attack: parsePersistedJson<AttackResult>(playerRecords[1].last_attack, createEmptyAttackResult())
            }];
            return players;
        });
        const gameState = {
