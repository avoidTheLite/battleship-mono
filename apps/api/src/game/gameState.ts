...
function parseStoredJson<T>(value: string | T): T {
    return typeof value === 'string' ? JSON.parse(value) : value;
}

function parseLastAttack(value: PlayerRecord['last_attack']): AttackResult {
    if (!value) {
        return {
            position: null,
            result: null,
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players = [{
                ...
                board_data: parseStoredJson<Board>(playerRecords[0].board_data),
                attack_data: parseStoredJson<Board>(playerRecords[0].attack_data),
                ship_data: parseStoredJson<ShipData>(playerRecords[0].ship_data),
                ...
                board_data: parseStoredJson<Board>(playerRecords[1].board_data),
                attack_data: parseStoredJson<Board>(playerRecords[1].attack_data),
                ship_data: parseStoredJson<ShipData>(playerRecords[1].ship_data),
