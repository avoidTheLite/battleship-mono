...
function createEmptyAttack(): AttackResult {
    return {
        position: null,
        result: null,
        target: null
...
function parseJsonField<T>(field: string | T | null, fallback: T): T {
    if (field === null) {
        return fallback;
    }
...
class GameStateController {
    ...
    public async createGame(players: PlayerBase[], gameName: string): Promise<GameState> {
        ...
        const player1: PlayerRecord = {
            ...
            last_attack: JSON.stringify(createEmptyAttack())
        }
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players = [{
                ...
                board_data: parseJsonField(playerRecords[0].board_data, createBoard()),
                attack_data: parseJsonField(playerRecords[0].attack_data, createBoard()),
                ship_data: parseJsonField(playerRecords[0].ship_data, createShips()),
                last_attack: parseJsonField(playerRecords[0].last_attack, createEmptyAttack())
            },{
                ...
                board_data: parseJsonField(playerRecords[1].board_data, createBoard()),
                attack_data: parseJsonField(playerRecords[1].attack_data, createBoard()),
                ship_data: parseJsonField(playerRecords[1].ship_data, createShips()),
                last_attack: parseJsonField(playerRecords[1].last_attack, createEmptyAttack())
            }];
            return players;
        });
        const gameState = {
