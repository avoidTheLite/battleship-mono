...
function createDefaultAttackResult(): AttackResult {
    return {
        position: null,
        result: null,
        target: null
...
function parseLastAttack(value: unknown): AttackResult {
    const parsedValue = parseJsonField<AttackResult | null>(value);
    return parsedValue ?? createDefaultAttackResult();
}
...
function convertPlayersToPlayerRecords(players: Player[]): PlayerRecord[] {
    ...
    for (let i = 0; i < players.length; i++) {
        const playerRecord: PlayerRecord = {
            ...
            last_attack: JSON.stringify(players[i].last_attack ?? createDefaultAttackResult())
        }
