...
function normalizeLastAttack(lastAttack: string | AttackResult | null): AttackResult {
    if (!lastAttack) {
        return createEmptyAttackResult();
    }
    const parsed = parseJsonField<Partial<AttackResult>>(lastAttack);
    return {
...
function convertPlayersToPlayerRecords(players: Player[]): PlayerRecord[] {
    ...
    for (let i = 0; i < players.length; i++) {
        const playerRecord: PlayerRecord = {
            ...
            last_attack: JSON.stringify(players[i].last_attack ?? createEmptyAttackResult())
        }
...
class GameStateController {
    ...
    public async getGame(gameID: string): Promise<GameState> {
        ...
        .then((playerRecords: PlayerRecord[]) => {
            ...
            const players: Player[] = playerRecords.map((playerRecord) => ({
                id: playerRecord.id,
