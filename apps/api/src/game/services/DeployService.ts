...
export default class DeployService {
    ...
    private isValidBoard(board: Board): boolean {
        ...
        const shipCounts: Record<Exclude<TargetKey, "O">, number> = {
            A: 0,
            B: 0,
            C: 0,
            S: 0,
        ...
        for (let i = 0; i < 10; i++) {
            ...
            for (let j = 0; j < 10; j++) {
                ...
                if (!Object.hasOwn(shipCounts, cell)) {
                    return false;
                }
                shipCounts[cell as Exclude<TargetKey, "O">] += 1;
            }
        }
        return (Object.keys(expectedShipCounts) as Exclude<TargetKey, "O">[])
            .every((shipKey) => shipCounts[shipKey] === expectedShipCounts[shipKey]);
    }
