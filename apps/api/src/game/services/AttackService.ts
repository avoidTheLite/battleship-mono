...
class AttackService {
    ...
    private getTargetIndex(shipData: Ship[], shipKey: ShipTargetKey): number {
        ...
        if (index === -1) {
            throw new AttackError({
                message: `Target marker ${shipKey} is missing from defender ship data`
            });
        }
        return index
    }

    private isShipTarget(target: string): target is ShipTargetKey {
        return target === "A" || target === "B" || target === "C" || target === "S" || target === "D";
    }
