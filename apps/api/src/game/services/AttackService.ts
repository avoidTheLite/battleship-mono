...
import type { Attack, Board, GameState, TargetKey, Ship, AttackResult } from "../../common/types/types.ts";
import { AttackError } from "../../common/types/errors.ts";
import TurnManager from "./TurnManager.ts";

const turnManager = new TurnManager();
const validShipTargets = new Set<TargetKey>(['A', 'B', 'C', 'S', 'D']);
...
class AttackService {
    ...
    public async attackCommand(gameID: string, attack: Attack): Promise<GameState> {
        ...
        if (!this.isValidAttack(attack)) {
            throw new AttackError({
                message: 'Invalid attack submitted. Must include a position between [0-9][0-9]'
            });
        }
        ...
        gameState.players[gameState.active_player_index].last_attack =
            gameState.players[gameState.active_player_index].last_attack || createEmptyAttackResult();
    ...
    private getTargetHit(defenderBoard: Board, coordinates: [number, number]): TargetKey {
        const targetHit = defenderBoard[coordinates[0]][coordinates[1]] as TargetKey;
        if (!validShipTargets.has(targetHit)) {
            throw new AttackError({
    ...
    private getTargetIndex(shipData: Ship[], shipKey: TargetKey): number {
        ...
        if (index === -1) {
            throw new AttackError({
                message: `Target ${shipKey} not found in defender ship data`
            });
        }
        return index
    }
