...
describe('Attack Service Test', () => {
    ...
    it('throws an error if the game is not in the play phase', async () => {
        const gameID = 'test';
        const attack: Attack = {
            position: [0, 0]
        };
    ...
    it('creates boards with independent rows', () => {
        const board = createBoard();
    ...
    it('persists a miss in the attacker attack data', async () => {
        const gameID = 'test';
        const attack: Attack = {
            position: [2, 3]
        };
    ...
    it('rejects attacks that were already recorded as misses', async () => {
        const attack: Attack = {
            position: [2, 3]
        };
        const gameState: GameState = createTestGame();
    ...
    it('rejects malformed attack coordinates without saving', async () => {
        const gameState: GameState = createTestGame();
        gameState.phase = 'play';
        mockGameStateController.getGame.mockResolvedValue(gameState);
    ...
    it('rejects invalid defender board markers instead of corrupting ship data', async () => {
        const attack: Attack = {
            position: [2, 3]
        };
        const gameState: GameState = createTestGame();
    ...
    it('rejects deploy boards with unknown ship markers', async () => {
    ...
    it('rejects deployment outside of deploy phase', async () => {
        const gameState: GameState = createTestGame();
        gameState.phase = 'play';
        mockGameStateController.getGame.mockResolvedValue(gameState);

        await expect(deployService.deployCommand('test', createValidDeployBoard())).rejects.toThrow(DeployError);
        expect(mockGameStateController.saveGame).not.toHaveBeenCalled();
    });

    it('rejects redeploying over an existing active player board', async () => {
        const gameState: GameState = createTestGame();
        gameState.players[0].board_data = createValidDeployBoard();
        mockGameStateController.getGame.mockResolvedValue(gameState);
